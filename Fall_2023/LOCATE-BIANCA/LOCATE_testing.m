...
train_image_directory_name = pwd;
...
disp(train_image_directory_name)
...
for subj = 1:numel(xdir)
    ...
    if does_file_exist == 2
        ...
        dim = size(lesionmask);
        factor = 1; %150./dim;%floor(max(dim)./dim);
        inv_factor = 1./factor;
...
save(sprintf('%s/LOCATE_test_features.mat',results_directory),'imgfeatmats',...
...
% as 'LOCATE_test_features.mat'
load(sprintf('%s/LOCATE_test_features.mat',results_directory))
